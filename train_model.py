...
class q_LSTM(nn.Module):
    ...
    def forward(self,question):
        ...
        out = [lstm_out[s][-1] for s in range(lstm_out.shape[0])]
        out_tensor = torch.stack(out, dim = 0).to(self.device) #[batch_size, hidden_dim]
        return out_tensor.to(self.device),question_embeds #return the last layer of lstm for each sentence
...
class CNN(nn.Module):
    def __init__(self, img_dim):
        ...
        self.layer3 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.MaxPool2d(2))
        self.img_dim = math.floor(((1/2**3)*img_dim))**2
...
class fully(nn.Module):
    def __init__(self, dictionary, hidden_size, CNN, class_num, word_dim=150, num_layers=2, img_size=299, drop = 0.5):
        ...
        self.i_weights = nn.Sequential(nn.Linear(hidden_size+img_dim_cnn,512),
                                     ...
                                     nn.Linear(512, 1),
                                     nn.Softmax(dim=1))
        self.q_weights = nn.Sequential(nn.Linear(word_dim+64*img_dim_cnn,512),
                                     nn.ReLU(),
                                     nn.Linear(512, 1),
                                     nn.Softmax(dim=1))
        self.fc_img = nn.Sequential(nn.Linear(img_dim_cnn, hidden_size),
                                    nn.ReLU())
        self.fc_q = nn.Sequential(nn.Linear(word_dim, hidden_size),
                                  nn.ReLU())
    ...
    def forward(self, q, i):
        q_out, question_embeds = self.lstm(q)
        ...
        # question att
        # image = i_out.view(i_out.size(0), -1)  # [batch_size, out_chanel*image_size/max_pool^2]
        # q_expand_dim = [question_embeds.shape[1],
        #               image.shape[0],
        #               image.shape[1]]
        # q_i_att = torch.cat((question_embeds, image.expand(q_expand_dim).permute(1, 0, 2)), dim=2)
        # q_weights = self.q_weights(q_i_att) #[batch_size , filter]
        # question_embeds = torch.mul(q_weights, question_embeds)
        # question_embeds = torch.sum(question_embeds, dim=1) #[batch, word_dim]
        # question_embeds = self.fc_q(question_embeds) #[batch_size, hidden_size]

        #fully
        out = torch.mul(q_out, i_out).to(self.device)
        out = self.relu(self.linear(out)).to(self.device)
        out = self.fc(out)
        return out.to(self.device)
...
def train(train_loader, val_loader, dictionary, hidden_size, class_num, epochs, word_dim=150, num_layers=2,
          img_size=299, lr=0.001, drop = 0.5):
