    ...
    for ans_entry in answers_dset:
        ...
        for k, v in answer_count.items():
            if k in ans2label:
                ...
                label_counts_new[ans2label[k]] = get_score(v)
        ...
        new_target[(ans_entry['question_id'], ans_entry['image_id'])] = label_counts_new

    print(cache_root)
    create_dir(cache_root)
    cache_file = os.path.join(cache_root, name + '_target.pkl')
    cache_new = os.path.join(cache_root, name + '_target_new.pkl')
    print(cache_file)
    with open(cache_file, 'wb') as f:
        cPickle.dump(target, f)
    with open(cache_new, 'wb') as f:
        cPickle.dump(new_target, f)
    return target
